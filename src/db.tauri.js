...
export async function syncStateWithServer({
  ...
  const requestBody = buildSyncRequestBody({
    deviceId,
  ...
  if (hasMore && nextCursor <= lastCursor) {
    throw new Error("Sync returned a non-advancing cursor while more changes remain");
  }
  ...
  await withWriteLock(async () => {
    ...
    await advanceLogicalClockFromSync(payload.serverTime, changes);
  });
  const pendingRows = await db.select(
    "SELECT 1 FROM sync_outbox WHERE device_id = $1 LIMIT 1",
    [deviceId]
  );
  ...
  return {
    ...
    nextCursor,
    hasMore,
    hasPendingMutations: pendingRows.length > 0,
  };
}

async function advanceLogicalClockFromSync(serverTime, changes) {
  ...
  let latest = toInt(serverTime, 0);
  for (const change of changes) {
    for (const field of timestampFields) {
      latest = Math.max(latest, toInt(change?.[field], 0));
    }
  }
  const current = toInt(await getMetaValue(SYNC_LOGICAL_CLOCK_META_KEY), 0);
  await setMetaValue(SYNC_LOGICAL_CLOCK_META_KEY, String(Math.max(current, latest)));
}
